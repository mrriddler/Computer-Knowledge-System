...
**GOT和PLT**：ELF通过.got\(global offset table\)来完成“间接”引用，.got就是一个指针数组，.got存储引用符号的实际地址。而代码段引用符号直接更改为引用.got项的位移，这在链接阶段以后就不会再改变了。然后将.got分配在.data section，装载时每个进程复制一份并修正。动态链接在装载的时候，根据符号表修正.got表项。动态链接库使用全局变量、静态变量都要经过.got。.plt同样是一个段，.plt存储对.got的引用。动态链接库引用文件外过程调用都要经过.plt。
