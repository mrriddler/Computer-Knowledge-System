...
反观静态链接，在链接阶段就可以确定出虚拟地址，而动态链接就不行了，主体程序无法“预测”出动态链接程序地址，动态链接也无法“预测”出自己程序地址，地址的确定要推迟到动态库装载后。这就使得引用外部动态链接程序和动态链接程序引用外部都要是**地址无关代码\(PIC\)，**地址无关代码指的就是代码引用地址不能引用绝对地址，要”间接“的引用，地址无关代码也不是个特定的技术，平常的指令就可能通过引用偏移而是这种地址无关代码。

### 地址无关代码\(PIC\)

地址无关代码有两种实现方式：

* GOT\(global offset table\)和PLT：其代码中对地址的引用改为引用特定的段，待动态链接库装载后会对该段进行修正。
* Rebasing和Binding：其代码中对地址的引用改为offset，待动态链接库装载后会对该offset添加相对地址，修正为绝对地址。

这两种思路的区别在于，是否能够让动态链接库物理共享。

**GOT和PLT**：ELF通过.got\(global offset table\)来完成“间接”引用，.got就是一个指针数组，.got存储引用符号的实际地址。而代码段引用符号直接更改为引用.got项的位移，这在链接阶段以后就不会再改变了。然后将.got分配在.data section，装载时每个进程复制一份并修正。动态链接在装载的时候，根据符号表修正.got表项。动态链接库使用全局变量、静态变量都要经过.got。.plt同样是一个段，.plt主要提供一个延时绑定的作用，.plt存储对.got的引用，第一次获取.plt中对应的.got符号是空的，经过符号决议后才会把结果存入.got，下次就可以直接使用。动态链接库引用文件外过程调用都要经过.plt。

动态链接库虽然在不同进程中有不同的映射虚拟空间，但物理空间上共享。.got在不同进程中，虚拟空间和物理空间都不共享。如下图所示：
...
**Rebasing和Binding：**dylib将所有对地址的引用生成.rel，等待动态库装载后，对所有.rel中的表项进行原地址和基地址的加操作，再将结果置换掉原地址引用。而这种思路就无法保证每个进程主程序可以共享动态库代码，每个进程要复制自己的动态库副本。

这两种思路相比而言，**GOT和PLT**保持共享的特点，节省了内存，而**Rebasing和Binding**仍然是直接引用，节省了时间。
